package com.lazibear.myintentapp;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        if(v.getId()==R.id.btn_move_activity){
            ...
            moveWithDataIntent.putExtra(MoveWithDataActivity.EXTRA_NAME, "HuahHuahBoy");
            moveWithDataIntent.putExtra(MoveWithDataActivity.EXTRA_AGE, 5);
            startActivity(moveWithDataIntent);
        }
        else if (v.getId()==R.id.btn_dial_number){
            String phoneNumber = "088889989908";
            Intent dialphoneIntent = new Intent(Intent.ACTION_DIAL, Uri.parse("tel:"  + phoneNumber));
            startActivity(dialphoneIntent);
        }else if (v.getId() == R.id.btn_move_activity_object) {
            Person person = new Person();
            person.setName("lazibear");
            person.setAge(21);
            person.setEmail("yambru");
            person.setCity("Salatiga");
